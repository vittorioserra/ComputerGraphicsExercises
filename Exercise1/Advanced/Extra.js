...
function mandelbrotSet(image) {
    for (let i = 0; i < 4 * image.width * image.height; i += 4) {
        ...
        let c = new ComplexNumberFromCoords(x, y, 'burningship_canvas');
...
function RenderMandelbrotSet() {
    // get the canvas
    let canvas = document.getElementById("burningship_canvas");
...
function onMouseDown(e) {
    ...
    if (e.ctrlKey) {
        ...
        if (firstLinePointSet == false) {
            ...
            secondLinePoint = [x, y];
            let c = document.getElementById('burningship_canvas');
...
function juliaLoop() {
    ...
    juliaC = new ComplexNumberFromCoords((1 - alpha) * firstLinePoint[0] + alpha * secondLinePoint[0], (1 - alpha) * firstLinePoint[1] + alpha * secondLinePoint[1], 'burningship_canvas');
    RenderJuliaSet();
    loopVariable++;
}
