...
function PhongLighting(context, point, normal, eye, pointLight, albedo, showVectors) {
    ...
    vec2.normalize(l,l);
    let r_l = vec2.create();
    let value = 2.0*(normal[0]*l[0]+normal[1]*l[1]);
    vec2.scale(r_l, normal, value);
    let r = vec2.create();
    r[0] = r_l[0]-l[0];
    r[1] = r_l[1] - l[1]; 
    ...
    let Iamb = vec3.fromValues(1.0, 1.0, 1.0);
    let Lamb = vec3.create();
    let amb = vec3.create();
    vec3.scale(amb, albedo, 0.1);
    Lamb[0] = amb[0] * Iamb[0];
    Lamb[1] = amb[1] * Iamb[1];
    Lamb[2] = amb[2] * Iamb[2];
    ...
    let Iin = vec3.fromValues(1.0, 1.0, 1.0);
    let Ldiff = vec3.create();
    let diff = vec3.create();
    vec3.scale(diff, albedo, 0.5);
    let scalar = normal[0]*l[0]+normal[1]*l[1];
    vec3.scale(diff, diff, scalar);
    Ldiff[0] = diff[0] * Iin[0];
    Ldiff[1] = diff[1] * Iin[1];
    Ldiff[2] = diff[2] * Iin[2];
    ...
    let shiny = 30.0;
    let Lspec = vec3.create();
    scalar = Math.pow((v[0]*r[0]+v[1]*r[1]),shiny);
    let spec = vec3.fromValues(0.4,0.4,0.4);
    vec3.scale(spec,spec,scalar);
    Lspec[0] = spec[0] * Iin[0];
    Lspec[1] = spec[1] * Iin[1];
    Lspec[2] = spec[2] * Iin[2];


    // 5. Add ambient, diffuse and specular color.
    //    Store the result in the variable color - replace the following dummy line:
    let color = vec3.create();
    color[0] = Lamb[0] + Ldiff[0] + Lspec[0];
    color[1] = Lamb[1] + Ldiff[1] + Lspec[1];
    color[2] = Lamb[2] + Ldiff[2] + Lspec[2];
...
function Basic1_2(canvas) {
    ...
    function Render() {
        ...
        for (let i = 0; i < nLineSegments; ++i) {
            ...
            let mid_point = vec2.create();  
            mid_point[1] = ((lineSegments[i][1][1]-lineSegments[i][0][1])/2)+lineSegments[i][0][1];
            mid_point[0] = ((lineSegments[i][1][0]-lineSegments[i][0][0])/2)+lineSegments[i][0][0];
                
            // 2. Compute the normal of the line segment.
            let norm = vec2.create();
            norm[1] = lineSegments[i][1][0]-lineSegments[i][0][0];
            norm[0] = -(lineSegments[i][1][1]-lineSegments[i][0][1]);
            ...
            let color_phong = PhongLighting(context, mid_point, norm, eye, pointLight, albedo, false)
            ...
            context.beginPath();
            setStrokeStyle(context,color_phong);
            context.lineWidth = 8;
...
function Basic1_3(canvas) {
    ...
    function Render() {
        ...
        for (let i = 0; i < nLineSegments; ++i) {
          ...
          normals_vertex = new Array(nLineSegments);
          ...
          for (let i = 0; i < nLineSegments; i++){
            ...
            let normal_s = normals_vertex[i];
            let normal_e = normals_vertex[i+1];
