...
//				have from Basic3.js!
uniform sampler2D textureCheckerboard;
...
void main(void) {
	...
	if(cobblestone) {
		...
		//reding rgb value from the normal

		vec2 texture_coord_scaled = uv;

		texture_coord_scaled[0] = uv[0]/planeSize[0];
		texture_coord_scaled[1] = uv[1]/planeSize[1];

		vec3 rgb_cobblestone = vec3(texture2D(cobblestoneTexture, texture_coord_scaled));
		rgb_cobblestone[0] = rgb_cobblestone[0] * 2.0 - 1.0;
		rgb_cobblestone[1] = rgb_cobblestone[1] * 2.0 - 1.0;
		rgb_cobblestone[2] = rgb_cobblestone[2] * 2.0 - 1.0;
		...
		color = vec3(texture2D(textureCheckerboard, uv));
	}
