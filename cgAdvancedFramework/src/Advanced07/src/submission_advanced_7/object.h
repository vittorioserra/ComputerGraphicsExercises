...
struct Object
{
    ...
    void interpolate(float alpha)
    {
        ...
        interpolatedTransformation.position = currentTransformation.position + alpha * (lastTransformation.position + (-1.0)*currentTransformation.position);
        interpolatedTransformation.scale = currentTransformation.scale + alpha*(lastTransformation.scale + (-1.0)*currentTransformation.scale);
        ...
        //interpolatedTransformation.scale = (1 - alpha) * lastTransformation.scale + alpha * currentTransformation.scale;
        //interpolatedTransformation.position = (1 - alpha) * lastTransformation.position + alpha * currentTransformation.position;
