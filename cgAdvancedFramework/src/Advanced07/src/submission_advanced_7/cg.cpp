...
void CG::decoupledMainLoop()
{
    ...
    while (running) {
        ...
        if(nextUpdate < currentTime){
            update(1.0/double(updateRate));//ticksPerUpdate_f/(1.0*SDL_GetPerformanceFrequency()));
            lastUpdate = nextUpdate;
            nextUpdate = nextUpdate + ticksPerUpdate;
        }

        //std::cout << "next update" << nextUpdate << "current ticks" << currentTime << std::endl;
		...
		if(nextRender < currentTime){
            ...
            if(interpolationMethod == 0) // No interpolation
                ...
                alpha += 1;
            // Change this line in order to use the correct dt.
            interpolate(float(ticksPerUpdate_f / double(1.0*SDL_GetPerformanceFrequency())), alpha);//ticksPerRender_f / (1.0*SDL_GetPerformanceFrequency()), alpha);
            render();
            imgui.beginFrame();
            renderGui();
            imgui.endFrame();
        ...
        //std::cout<<"wait time : "<<waitTime<<std::endl;
         // Code for sleeping:
        double w = double(waitTime) / double(SDL_GetPerformanceFrequency());
