...
void main() {
    ...
    vec4 wp = inverse(projView) * vec4(position, 1.0); // <-- replace this line

    // 2. Compute view direction.
	//	  Use the variable 'cameraPos' to do so.
    vec3 direction = normalize(vec3(wp/wp.w)-cameraPos); // <-- replace this line

    // 3. Convert view direction to texture coordinates and read from the 'color' texture.
    vec2 angles = cartesianToSpherical(direction);

    vec2 tc = sphericalToTexture(angles); // <-- replace this line
